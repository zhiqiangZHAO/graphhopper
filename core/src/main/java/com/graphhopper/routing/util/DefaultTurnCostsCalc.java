...
public class DefaultTurnCostsCalc implements TurnCostCalculation {
    ...
    /**
     * Creates a new {@link DefaultTurnCostsCalc} by the given mode of
     * operation.
     *
    ...
    /**
     * Creates a new {@link DefaultTurnCostsCalc} which determines its mode of
     * operation by {@link EdgePropertyEncoder} and {@link WeightCalculation}.
     * When using {@link BikeFlagEncoder} turn costs for bike will be
     * considered, when using {@link CarFlagEncoder} turn costs for cars will be
     * considered. If we use {@link FastestCalc} as weight calculation (i.e. we
     * are searching for fastest routes), we consider turn restrictions AND
     * costs. If we use another weight calculation (e.g. we are searching for
     * shortest routes) we do not need to consider turn costs but turn
     * restrictions only.
     *
     * @param edgeEncoder
     * @param weightCalc
