 ...
 * 
 * int - the 32 bits are parted from left (high) to right (low) bits as following: 4 bits - turn
 * costs flags (e.g. fourth bit means 'restriction for cars', third bit means 'restriction for
 * bike', first and second bit are free for later use) 14 bits - costs describing the time this turn
 * would cost by using a bike 14 bits - costs describing the time this turn would cost by using a
 * car
 * 
 * @author Karl Hübner
 * 
...
public final class TurnCostEncoder
{
    ...
    public static int costs( double costsBike, double costsCar, int flags )
    {
        if ( costsCar < 0 || costsCar > COSTS_MAX )
        {
            throw new IllegalArgumentException(
                    "time of the turn by car must be bigger or equal than 0 and less or equal than 4095");
        }

        if ( costsBike < 0 || costsBike > COSTS_MAX )
        {
            throw new IllegalArgumentException(
                    "time of the turn by bike must be bigger or equal than 0 and less or equal than 4095");
        }
     ...
     * @return <code>true</code> if this turn is restricted in both fastest and shortest routing and
    ...
    /**
     * @param flags holds the costs of this turn
     * @return <code>true</code> if this turn is restricted in both fastest and shortest routing for
     *         bikes
     */
